...
import { MapPin, Bell, Search, Trophy, Sparkles, Flame, X, MessageSquareText, Calendar, Info, Star, ShieldAlert, BadgePercent, LayoutGrid } from 'lucide-react';
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      {isSearchFocused ? (
        ...
        /* Normal Home Content */
        <>
            {/* Quick Access Grid (Updated: 4 items interactable simultaneously) */}
            <div className="mt-4 px-4 mb-6">
                <div className="grid grid-cols-4 gap-3">
                    <div 
                        onClick={() => navigate('/leaderboard?metric=rating')}
                        className="flex flex-col items-center cursor-pointer group"
                    >
                        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-red-100 to-orange-100 text-red-500 flex items-center justify-center mb-2 shadow-sm border border-red-50 group-active:scale-95 transition-transform">
                             <ShieldAlert size={20} />
                        </div>
                        <span className="text-[10px] font-bold text-gray-700 text-center leading-tight">拒绝<br/>踩雷</span>
                    </div>

                    <div 
                        onClick={() => navigate('/leaderboard?metric=popularity')}
                        className="flex flex-col items-center cursor-pointer group"
                    >
                        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-yellow-100 to-amber-100 text-yellow-600 flex items-center justify-center mb-2 shadow-sm border border-yellow-50 group-active:scale-95 transition-transform">
                             <Flame size={20} />
                        </div>
                        <span className="text-[10px] font-bold text-gray-700 text-center leading-tight">好评<br/>如潮</span>
                    </div>

                    <div 
                        onClick={() => navigate('/leaderboard?metric=price')}
                        className="flex flex-col items-center cursor-pointer group"
                    >
                         <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-green-100 to-emerald-100 text-green-600 flex items-center justify-center mb-2 shadow-sm border border-green-50 group-active:scale-95 transition-transform">
                             <BadgePercent size={20} />
                        </div>
                        <span className="text-[10px] font-bold text-gray-700 text-center leading-tight">性价比<br/>之选</span>
                    </div>
                    ...
                    <div 
                        ...
                        className="flex flex-col items-center cursor-pointer group"
                    >
                        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-100 to-indigo-100 text-blue-500 flex items-center justify-center mb-2 shadow-sm border border-blue-50 group-active:scale-95 transition-transform">
                             <LayoutGrid size={20} />
                        </div>
                        <span className="text-[10px] font-bold text-gray-700 text-center leading-tight">应有<br/>尽有</span>
                    </div>
                </div>
            </div>
